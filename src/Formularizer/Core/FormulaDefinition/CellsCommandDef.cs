...
{
	public class CellsCommandDef : SelectorCommandDef
	{
		private static readonly Regex CommandRegex =
			new Regex(@"cells\s*'(?<filter>[\w\s,.*+-]+)'\s*(?<scope>[\w]+)\s*('(?<sheet>[\w\s(),.*+-]+)')*",
				RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase);

		private CellsCommandDef(string sheetName, bool useOwnId, string filter, SelectorScope scope, string reference)
		...
		public static Result<IFormulaPartDef> Create(string rawText, string reference)
		{
			...
			if (!match.Success)
			{
				return Result.Fail<IFormulaPartDef>(string.Format("Incorrect syntax of 'cells' command: '{0}'", rawText));
			}
			...
			return Result.Ok((IFormulaPartDef) new CellsCommandDef(sheetName, useOwnId, filter, scope, reference));
		}
	}
}